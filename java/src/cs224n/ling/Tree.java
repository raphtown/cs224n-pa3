...
public class Tree<L> implements Serializable, Decodable {
	...
	public Pair<Tree<L>, Pair<Integer, Integer>> getTreeAtPath(List<Pair<L, Integer>> pIn) {
		List<Pair<L, Integer>> p = new ArrayList<Pair<L, Integer>>();
		p.addAll(pIn);
		p.remove(0); // Get rid of root node
		return getTreeAtPathHelper(p, Pair.make(0, this.getYield().size()));
	}

	private  Pair<Tree<L>, Pair<Integer, Integer>> getTreeAtPathHelper(List<Pair<L, Integer>> p, Pair<Integer, Integer> currRange) {
		//--Base Case
		if(p.isEmpty()){
			return Pair.make(this, currRange);
		}
		//--Recursive Case
		//(get children)
		int currIndex = currRange.getFirst();
		for(Tree<L> child : getChildren()) {
			if (child.getUniqueIndex() == p.get(0).getSecond()) {
				p.remove(0);
				return child.getTreeAtPathHelper(p, Pair.make(currIndex, child.getYield().size() + currIndex));
			}
			currIndex = child.getYield().size() + currIndex;
		}
		return null;
	}
